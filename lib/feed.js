...
function parseArrivals(stop) {
  const { routeId, borough, stopName, groups } = stop;
  const arrivals = groups.filter(({ route }) => route.id === routeId).reduce((acc, { headsign, times }) => {
    return Object.assign(acc, { [headsign]: times.map(({ arrivalFmt }) => arrivalFmt).shift() });
  }, {});
  return {
    borough,
    stopName,
